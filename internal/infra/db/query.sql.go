...
const findErasedMessageByID = `-- name: FindErasedMessageByID :many
...
func (q *Queries) FindErasedMessageByID(ctx context.Context, chatID string) ([]Message, error) {
	rows, err := q.db.QueryContext(ctx, findErasedMessageByID, chatID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const findMessageByID = `-- name: FindMessageByID :many
...
func (q *Queries) FindMessageByID(ctx context.Context, chatID string) ([]Message, error) {
	rows, err := q.db.QueryContext(ctx, findMessageByID, chatID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
