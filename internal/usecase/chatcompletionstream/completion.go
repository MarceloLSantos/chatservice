...
type ChatCompletionConfigInputDTO struct { //DTO = Data Transfer Object (guarda valores para transitar entre camadas)
	Model                string
	ModelMaxTokenx       int
	Temperature          float32
	TopP                 float32
...
func (uc *ChatCompletionUseCase) Execute(ctx context.Context, input ChatCompletionInputDTO) (*ChatCompletionOutputDTO, error) {
	...
	if err != nil {
		if err.Error() == "chat not found" {
			...
			if err != nil {
				return nil, errors.New("error pesisting new chat: " + err.Error())
			}
		} else {
	...
	var fullResponse string.Builder
